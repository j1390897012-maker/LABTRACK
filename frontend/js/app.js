...
function mostrarVista(nombre, nombreBreadcrumb = null) {
  ...
  if (vistaActual && vistaActual !== "prestamo-rfid" && vistaActual !== nombre) {
  ...
  navButtons.forEach((item) => {
    item.classList.toggle("active", item.dataset.view === nombre);
  });

  if (breadcrumb) {
    breadcrumb.textContent = nombreBreadcrumb || viewNames[nombre] || nombre;
  }
}
...
function abrirVistaPrestamoRFID() {
  ...
  if (vistaActual && vistaActual !== "prestamo-rfid") {
    vistaAnterior = vistaActual;
  }
  ...
  const vista = document.getElementById("view-prestamo-rfid");

  if (!vista) {
    console.error("No existe la vista view-prestamo-rfid");
    return;
  }
...
function regresarDesdePrestamoRFID() {
  ...
  mostrarVista(destino, viewNames[destino] || destino);
}
